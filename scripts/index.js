    ...
    db.collection('weekendevents').get()
        .then(async snap => {
            ...
            snap.forEach((doc) => {
                var data = doc.data();
                /* Weekly Change */ 
                if ((data.week > 13) && (data.month === "01" || data.month === "02" || data.month === "03" || data.month === "04" || data.month === "05") && (data.status === "approved") && (!data.type)){ 
                ...
                /* Weekly Change */ 
                if ((data.week < 14) && (data.status === "approved") || (data.type)){
...
const setupEvents = (data, num, isLive) => {
   ...
   // console.log(data)
    ...
    data.forEach(event => {
        ...
        var opacity = ""
        /* Weekly Change */ 
        if ((event.week < 14) && (event.status === "approved") || (event.type)){
