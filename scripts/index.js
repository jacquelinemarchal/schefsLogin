    ...
    db.collection('weekendevents').get()
        .then(async snap => {
            ...
            snap.forEach((doc) => {
                var data = doc.data();
                /* Weekly Change */ 
                if ((data.week > 1 && data.week < 5) && (data.month === "01" || data.month === "02") && (data.status === "approved")){ 
                ...
                /* Weekly Change */ 
                if ((data.week === 1 || data.week === 2 || data.week === 3 || data.week === 4 || data.week === 5  || data.week === 7 || data.week === 8 || data.week === 9) && (data.status === "approved") && (data.month > "02")){
...
const setupEvents = (data, num, isLive) => {
   ...
   // console.log(data)
    ...
    data.forEach(event => {
        ...
        var opacity = ""
        /* Weekly Change */ 
        if ((event.week == 1 || event.week == 2 || event.week == 3 || event.week == 4 || event.week == 5 || event.week == 6 || event.week === 7 || event.week === 8 || event.week === 9) && event.month > "02") {
