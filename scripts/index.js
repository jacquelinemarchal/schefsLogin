...
const renderHomeEvents = async () => {
    ...
    db.collection('weekendevents').get()
        .then(async snap => {
            ...
            snap.forEach((doc) => {
                var data = doc.data();
                /* Weekly Change */  // minus one
                if ((data.week > 18) && (data.month === "01" || data.month === "02" || data.month === "03" || data.month === "04" || data.month === "05" || data.month === "06") && (data.status === "approved") && (!data.type)){ 
                ...
                /* Weekly Change */ 
                if ((data.week <= 18) && (data.status === "approved") || (data.type)){ // plus one
...
const setupEvents = (data, num, isLive) => {
   ...
   // console.log(data)
    ...
    data.forEach(event => {
        ...
        var opacity = ""
        /* Weekly Change */ 
        if ((event.week <= 18) && (event.status === "approved") || (event.type)){
